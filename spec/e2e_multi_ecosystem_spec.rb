...
RSpec.describe 'Multi-Ecosystem E2E Tests', :e2e do
  ...
  describe 'Python Packages' do
    ...
    it 'detects namespace squatting in Python packages' do
      components = [
        { 'purl' => 'pkg:pypi/django@4.2.0' },
        { 'purl' => 'pkg:pypi/django-test-helpers@0.1.0' },
      ]
      ...
      results[:results].find { |r| r[:package][:name] == 'django-test-helpers' }
    end
  end
  ...
  describe 'Go Modules' do
    it 'verifies popular Go packages' do
      ...
      expect(gin_result[:trust][:score]).to be >= 50
      expect(gin_result[:trust][:level]).to match(/MEDIUM|HIGH/)
    ...
    it 'scores packages based on OpenSSF Scorecard' do
      ...
      scorecard_component = gin_result[:trust][:breakdown].find do |b|
        ['openssf_scorecard', 'scorecard'].include?(b[:signal])
      end

      expect(scorecard_component[:points]).to be >= 0 if scorecard_component
    end
    ...
    it 'weighs GitHub stars in scoring' do
      ...
      expect(stars_component[:points]).to be >= 11 if stars_component
    end
  end
  ...
  describe 'Mixed Ecosystems' do
    it 'handles SBOM with Ruby, Python, and Go packages' do
      ...
      results[:results].each do |result|
        expect(result[:action]).not_to eq('BLOCK')
        expect(result[:action]).not_to eq('NOT_FOUND')
      end
    end
