...
module SlopGuard
  class EcosystemAdapter
    ...
    def extract_github_url(_metadata)
      nil
    end
    ...
    def score_downloads(download_count, critical: 100_000_000, high: 10_000_000, medium: 1_000_000, max_points: 30)
      ...
      if download_count > critical
        score = max_points
        breakdown << { signal: 'downloads', points: max_points,
reason: "Critical infrastructure (#{format_count(download_count)} downloads)" }
      elsif download_count > high
        score = (max_points * 0.8).round
        breakdown << { signal: 'downloads', points: score,
reason: "Widely used (#{format_count(download_count)} downloads)" }
      elsif download_count > medium
        score = (max_points * 0.5).round
        breakdown << { signal: 'downloads', points: score,
reason: "Popular (#{format_count(download_count)} downloads)" }
      elsif download_count > 100_000
        score = (max_points * 0.3).round
        breakdown << { signal: 'downloads', points: score,
reason: "Moderate usage (#{format_count(download_count)} downloads)" }
      end
