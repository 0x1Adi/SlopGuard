...
module SlopGuard
  class Reporter
    ...
    def self.text_report(results)
      ...
      if results[:high_risk].positive? || results[:not_found].positive?
        messages = []
        messages << "#{results[:high_risk]} high-risk" if results[:high_risk].positive?
        messages << "#{results[:not_found]} non-existent" if results[:not_found].positive?
        lines << "❌ FAILED: Found #{messages.join(' and ')} packages"
      elsif results[:suspicious].positive?
